...
public class NSwagModule : BaseModule
{
    ...
    {
        context.Logger.LogInformation("UseOpenApi");

        var app = context.App;
        var options = app.Services.GetRequiredService<IOptions<NSwagOptions>>().Value;
        ...
        if (options.EnableSwaggerUI)
        {
            ...
            {
                UseSwaggerUIPasswordProtection(app,options);
            }
    ...
    /// </summary>
    private void UseSwaggerUIPasswordProtection(
    ...
    /// 处理密码验证请求
    /// </summary>
    private async Task HandlePasswordVerification(HttpContext context, NSwagOptions options)
    {
    ...
    /// 渲染密码输入页面
    /// </summary>
    private async Task RenderPasswordPage(HttpContext context, NSwagOptions options)
    {
