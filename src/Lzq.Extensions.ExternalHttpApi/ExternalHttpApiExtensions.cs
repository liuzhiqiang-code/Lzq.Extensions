...
public static class ExternalHttpApiExtensions
{
        ...
        IConfiguration configuration)
    {
        ...
        return AddExternalHttpApis(services, configuration, assembliesToScan.ToArray());
    }
    ...
    public static IServiceCollection AddExternalHttpApis(this IServiceCollection services,IConfiguration configuration,params Assembly[] assembliesToScan)
    {
        ...
        var apiInterfaces = assembliesToScan
            ...
            .Where(t => t.IsInterface
                ...
                && typeof(IExternalHttpApi).IsAssignableFrom(t)
                && t != typeof(IExternalHttpApi)) // 关键：排除掉定义接口本身)
            .ToList();
        ...
        foreach (var apiType in apiInterfaces)
        {
            ...
            }).ConfigureHttpClient(c => c.Timeout = TimeSpan.FromSeconds(apiConfigSection["Timeout"] != null ? double.Parse(apiConfigSection["Timeout"] ?? "100") : 100)); ;
        }
